...
This is a thin wrapper over futures_engine.py so there is ONE source of truth:
every render_* function (to_win, playoffs, team_points, awards, props,
fd_desk, team_view, player_view), the shared comparison-table renderer, and
the price-history popovers all live in futures_engine.py and are driven by
render_comp_tool(). (app.py is the hub entry point Streamlit actually runs —
the engine lives in its own file so app.py can own top-level nav/page config.)
...
Integration notes (hub / Databricks):
  ...
  * `data` is the full odds.json (dict). `history` is the optional
    ...
    Volume rather than on local disk next to futures_engine.py.
...
from futures_engine import render_comp_tool  # noqa: F401  (re-export the single entry point)
